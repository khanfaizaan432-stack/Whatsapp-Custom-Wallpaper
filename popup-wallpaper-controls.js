// =============================================================================
// WhatsApp Themes — popup-wallpaper-controls.js v1.8.0
// Adds wallpaper fit/position/zoom/dim/filter controls to the popup.
// =============================================================================
(() => {
  'use strict';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .wa-wallpaper-field label {
        color:#8696a0;
        font-size:10.5px;
        font-weight:700;
      }
      .wa-wallpaper-field select,
      .wa-wallpaper-field input[type="range"] {
        width:100%;
      }
      .wa-wallpaper-field select {
        background:#1f2c33;
      ...
      #waWallpaperControlsPreview {
        ...
        background:
          radial-gradient(circle at 20% 20%, rgba(0,168,132,.75), transparent 30%),
          radial-gradient(circle at 80% 10%, rgba(109,93,252,.65), transparent 28%),
          linear-gradient(135deg, #0b141a, #1f2c33);
        overflow:hidden;
      }
      #waWallpaperControlsPreview::after {
        content:"";
        position:absolute;
        inset:0;
        background:rgba(0,0,0,var(--wa-preview-dim,0));
      }
      #waWallpaperControlsPreview::before {
        content: attr(data-fit) " / " attr(data-position) " / " attr(data-zoom) "%";
        position:absolute;
        left:8px;
        bottom:7px;
        z-index:2;
  ...
  function buildPanel() {
    ...
    panel.id = PANEL_ID;
    panel.innerHTML = `
      <div class="wa-wallpaper-head">
        <div>
          <div class="wa-wallpaper-title">Wallpaper Controls</div>
          <div style="font-size:10.5px;color:#8696a0;margin-top:2px;">Crop, position, zoom, dim, and filters for the chat wallpaper.</div>
        </div>
        <div id="waWallpaperControlsState">Ready</div>
      </div>
      <div id="waWallpaperControlsPreview" data-fit="cover" data-position="center" data-zoom="100"></div>
      <div class="wa-wallpaper-grid">
        <div class="wa-wallpaper-field">
          <label for="waWallpaperFit">Fit</label>
          <select id="waWallpaperFit">
        ...
        <div class="wa-wallpaper-field">
          <label for="waWallpaperPosition">Position</label>
          <select id="waWallpaperPosition">
      ...
      <div class="wa-wallpaper-actions">
        <button type="button" class="wa-wallpaper-btn primary" id="waWallpaperSaveControls">Apply wallpaper controls</button>
        <button type="button" class="wa-wallpaper-btn" id="waWallpaperResetControls">Reset controls</button>
      </div>
    `;
  ...
  function wireEvents() {
    ...
    for (const id of ids) {
      ...
      control.addEventListener('input', () => {
        updateLabels();
        updatePreviewSwatch();
        byId('waWallpaperControlsState')?.replaceChildren(document.createTextNode('Unsaved'));
      });
    ...
    byId('waWallpaperSaveControls')?.addEventListener('click', () => applyControls().catch(error => {
      byId('waWallpaperControlsState')?.replaceChildren(document.createTextNode(`Error: ${error.message}`));
    }));
    byId('waWallpaperResetControls')?.addEventListener('click', () => resetControls().catch(error => {
      byId('waWallpaperControlsState')?.replaceChildren(document.createTextNode(`Error: ${error.message}`));
    }));
  }
